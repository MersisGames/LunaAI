...
def search_in_long_term_memory(question, data):
    ...
    def calculate_similarity(x):
        try:
            x_vector = np.array(eval(x))
            return cosine_similarity(question_vectorized.reshape(1, -1), x_vector.reshape(1, -1))[0][0]
        except Exception as e:
            return 0.0  # Return a low similarity in case of an error

    data['Similarity'] = data['Vector_Question'].apply(calculate_similarity)
...
def generate_response_with_long_term_memory(question, similar_response, data, conversacion_data, ST_memory_filename):
    ...
    new_data = pd.DataFrame({'Question': [question], 'Response': [LTM_response], 'Vector_Question': [vectorizer.embed(question)], 'Vector_Response': [vectorize_text(LTM_response)]})
...
def create_response_with_lunaData(question, data, max_lines=3):
    ...
    new_data = pd.DataFrame({'Question': [question], 'Response': [luna_response], 'Vector_Question': [vectorizer.embed(question)], 'Vector_Response': [vectorizer.embed(luna_response)]})
    data = pd.concat([data, new_data], ignore_index=True)
    data.to_csv(data_filename, index=False)
